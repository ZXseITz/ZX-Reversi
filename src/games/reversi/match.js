...
const {BoardIterator} = require('./boardIterator');
...
class Match {
    ...
    constructor(id, white, black) {
        ...
        this.history = []
        this.iterators = {
            RIGHT: new BoardIterator(this.board, {x: 1, y: 0}),
            RIGHT_UP: new BoardIterator(this.board, {x: 1, y: 1}),
            UP: new BoardIterator(this.board, {x: 0, y: 1}),
            LEFT_UP: new BoardIterator(this.board, {x: -1, y: 1}),
            LEFT: new BoardIterator(this.board, {x: -1, y: 0}),
            LEFT_DOWN: new BoardIterator(this.board, {x: -1, y: -1}),
            DOWN: new BoardIterator(this.board, {x: 0, y: -1}),
            RIGHT_DOWN: new BoardIterator(this.board, {x: 1, y: -1}),
        }
        this.white = white;
        this.black = black;
        this.count = 4;
        this.state = GameStates.NEXT_BLACK;
    }
     ...
     * @param {BoardIterator} iterator - field iterator
    ...
    set(player, point) {
        ...
        if (this.board.covers(point) && this.board.get(point) === 0) {
            const fields = [
            ...
            if (fields.length > 0) {
                ...
                this.state = GameStates.NEXT_WHITE;
